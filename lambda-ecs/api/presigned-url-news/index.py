...
def lambda_handler(event, context):
    """
    Lambda function to generate presigned URLs for news uploads to S3 news bucket
    """
    
    try:
        # Get environment variables
        s3_bucket = os.environ['S3_NEWS_BUCKET']
        ...
        if not any(file_name.lower().endswith(ext) for ext in allowed_extensions):
            return {
                ...
                'body': json.dumps({
                    'error': 'Only TXT, MD, and JSON files are allowed'
                })
            }
